...
//     }),
//     onSubmit: (values: any) => {
//         // This dispatches the login action which now uses your API endpoint
//         dispatch(loginUser(values, props.router.navigate));
//     }
// });
import React from "react";
...
const Login = (props: any) => {
    ...
    const validation: any = useFormic({
        enableReinitialize: true,
        initialValues: {
            username: user.username || "admin" || '',  // Updated to use "username"
            password: user.password || "123456" || '',
        },
        validationSchema: Yup.object({
            username: Yup.string().required("Please Enter Your Username"),  // Updated validation
            password: Yup.string().required("Please Enter Your Password"),
        }),
        onSubmit: (values: any) => {
            dispatch(loginUser(values, props.router.navigate));
                        ...
                        >
                            ...
                            {error && (
                                <div className="px-4 py-3 mb-3 text-sm text-red-500 border border-red-200 rounded-md bg-red-50 dark:bg-red-400/20 dark:border-red-500/50" id="errorAlert">
                                    You have <b>failed</b> to sign in.
                                </div>
                            )}

                            {/* Username Input */}
                            <div className="mb-3">
                                <label htmlFor="username" className="inline-block mb-2 text-base font-medium">Username</label> {/* Updated to "Username" */}
                                <input
                                    type="text"
                                    id="username"
                                    name="username"
                                    className="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 dark:bg-zink-700 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                    placeholder="Enter username"  // Updated placeholder to "Enter username"
                                    onChange={validation.handleChange}
                                    onBlur={validation.handleBlur}
                            ...
                            <div className="mt-10">
                                <button type="submit" className="w-full text-white btn bg-custom-500 border-custom-500 hover:bg-custom-600">
                                    Sign In
                                </button>
                            </div>
                            ...
                            <div className="mt-10 text-center">
                                <p className="mb-0 text-slate-500 dark:text-zink-200">
                                    Don't have an account? <Link to="/register" className="font-semibold underline text-slate-500 dark:text-zink-200"> Sign Up</Link>
                                </p>
                            </div>
                        </form>
                    </div>
